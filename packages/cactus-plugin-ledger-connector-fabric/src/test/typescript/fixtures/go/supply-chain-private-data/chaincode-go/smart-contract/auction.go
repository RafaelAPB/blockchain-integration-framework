/*
SPDX-License-Identifier: Apache-2.0 - Adapted from https://github.com/hyperledger/fabric-samples/tree/main/auction
*/

package supplychainviews
...
//Each stakeholder/organization starts with a certain stock
//Maps asset IDs to the number of assets
type Organization struct {
	OrgId	string	`json:"orgId"`
	ItemStock map[string]uint `json:"stock"`
}

func (s *SmartContract) TransferAsset(ctx contractapi.TransactionContextInterface) (string,error)	{
	return "1", nil
}
// Creates one company for each Stakeholder (types 1-5)
func (s *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface)	([]Organization, error) {
	fmt.Println("Initializing Ledger")

	supplier := Organization{
		OrgId: "1",
		ItemStock: make(map[string]uint),
	}
	...
	err = ctx.GetStub().PutState(supplier.OrgId,supplierJSON)
	if err != nil	{
		return nil, err
	}
	...
	err = ctx.GetStub().PutState(distributor.OrgId,distributorJSON)
	if err != nil	{
		return nil, err
	}
	...
	err = ctx.GetStub().PutState(shipper.OrgId,shipperJSON)
	if err != nil	{
		return nil, err
	}
	...
	wholesalerJSON, err := json.Marshal(wholesaler)
	if err != nil {
		return nil, err
	}

	err = ctx.GetStub().PutState(wholesaler.OrgId,wholesalerJSON)
	if err != nil	{
		return nil, err
	}
	...
	err = ctx.GetStub().PutState(retailer.OrgId,retailerJSON)
	if err != nil	{
		return nil, err
	}

	fmt.Println("Orgs created")
	fmt.Println("Minting Asset 1")

	orgs := []Organization{supplier,shipper,distributor,wholesaler,retailer}
	return orgs, nil
}


func (s *SmartContract) ProduceAsset(ctx contractapi.TransactionContextInterface, CreatorOrg string, assetType string, unitAmount uint)  (string, error)	{
	//Obtain state of the creator
	log.Println("Obtaining state from %s", CreatorOrg)
	fmt.Println("Obtaining state from %s", CreatorOrg)

	creatorBytes, err := ctx.GetStub().GetState(CreatorOrg)
	
	if err != nil {
		jsonResp := "{\"Error\":\"Failed to get creatorBytes at Produce Asset\"}"
		log.Fatal(jsonResp)
		return "nil", err
	}

	var organization Organization
	json.Unmarshal(creatorBytes, &organization)
	log.Println("Obtained organization: %v", organization)

	if amount, ok := organization.ItemStock[assetType]; ok	{
		fmt.Println("Asset item", assetType, " exists, there are: ", amount)
		organization.ItemStock[assetType] += unitAmount
	}	else	{
		fmt.Println("Asset item", assetType, " does NOT exist. Adding")
		organization.ItemStock[assetType] = unitAmount
	}

	
	updatedOrgJSON, err := json.Marshal(organization)
	if err != nil {
		return "nil", err
	}

	err = ctx.GetStub().PutState(organization.OrgId,updatedOrgJSON)
	if err != nil	{
		return "nil", err
	}

	response := fmt.Sprintf("Organization ", organization.OrgId, " created ", organization.ItemStock[assetType], " units of ", assetType)
...
func (s *SmartContract) CreateAuction(ctx contractapi.TransactionContextInterface, clientID string, auctionID string, itemsold string) error {
...
func (s *SmartContract) RevealBid(ctx contractapi.TransactionContextInterface, clientID string, auctionID string, txID string) error {
...
func (s *SmartContract) CloseAuction(ctx contractapi.TransactionContextInterface, clientID string, auctionID string) error {
...
// of the auction
func (s *SmartContract) EndAuction(ctx contractapi.TransactionContextInterface, clientID string, auctionID string) error {
