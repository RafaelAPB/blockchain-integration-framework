...
func (s *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface)	(string, error) {
	...
	orgs := []Organization{
		{OrgId: "supplier", ItemStock: make(map[string]uint)},
		{OrgId: "distributor", ItemStock: make(map[string]uint)},
		{OrgId: "shipper", ItemStock: make(map[string]uint)},
		{OrgId: "wholesaler", ItemStock: make(map[string]uint)},
		{OrgId: "retailer", ItemStock: make(map[string]uint)},
	}
	...
	for _,organization := range orgs	{
		...
		err = ctx.GetStub().PutState(organization.OrgId, orgJSON)
		if err != nil	{
			return "failed to put to world state.", err
		}

		fmt.Println("Org ", organization.OrgId, " added. ")
	}

	fmt.Println("Initialized ledger with ", len(orgs), " orgs")

	initializedOrg := "supplier"
	unitType := "asset1"
	unitAmmount := uint(100)

	s.ProduceAsset(ctx,initializedOrg ,unitType, unitAmmount)

	fmt.Println("Initialized",  initializedOrg, "with ", unitAmmount, "units of ", unitType)

	return "Initialized ledger ", nil
}
